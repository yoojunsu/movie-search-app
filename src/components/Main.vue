<template>
  ...
  <section class="content-section nowplaying-section">
    <div class="section-inner">
      ...
      <div class="btn-wrap">
        <button type="button" :class="{ active: Genre }">MOVIE</button>
        <button type="button" :class="{ active: Genre === false }">TV</button>
      </div>
      <div class="content-box">
        <div class="scr-box">
          <ul class="card-list">
...
export default {
  ...
  watch: {
    // Genre State값 변경 감시 watcher
    Genre(genre) {
      genre === "movie" ? true : false;
    },
  },
  methods: {
    ...
    randomVisualBg() {
      ...
      const mainRandomVisualImageOptions =
        "w1920_and_h600_multi_faces_filter(duotone,000,00baff)";
